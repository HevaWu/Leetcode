...
class CombinationIterator {
    ...
    all combination bitmask array
    var items = [Int]()
    var cur = 0

    init(_ characters: String, _ combinationLength: Int) {
        charArray = Array(characters)
        size = combinationLength

        helper(0, size, 0, characters.count)
        print(items)
    }

    backtracking build bitmask
    func helper(_ index: Int, _ remain: Int,
                _ mask: Int, _ n: Int) {
        if remain == 0 {
            items.append(mask)
            return
        ...
        if index == n || n-index < remain { return }
