...
which is 0. Subscribe to see which companies asked this question*/

/*use "remain" to keep the size of the ret string
start from the begin of string, if current int smaller than former int, pop
former character out, push current in after pushing, resize to shorten the size
to "remain" size then remove the first 0*/
class Solution {
 public:
  string removeKdigits(string num, int k) {
    ...
    for (int i = 0; i < num.size(); ++i) {
      while ((num.size() - i > remain - index) && index > 0 &&
             ret[index - 1] > num[i]) {
        index--;
      }
