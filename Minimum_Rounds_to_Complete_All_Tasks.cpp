...
- for others,
  ...
  - check task % 3
    ...
    - if it is 1, return rounds + 1
      - can group it by (rounds-1) with 3 tasks, then have 3+1 remaining, can
group it by 2 rounds with 2 tasks
...
class Solution {
 public:
  int minimumRounds(vector<int>& tasks) {
    ...
    int rounds = 0;
    for (unordered_map<int, int>::iterator it = group.begin();
         it != group.end(); it++) {
      int task = it->second;
      if (task == 1) {
        return -1;
  ...
  int countRounds(int task) {
    int rounds = task / 3;
    // task%3 could be either 0, 1, 2
    if (task % 3 == 0) {
      return rounds;
    } else {
      // for task%3 == 1, can complete it by rounds-1 *3  + 2 * 2
      // ex: task = 4, can complete it by 2 * 2
      // ex: task = 7, can complete it by 3 * 1 + 2 * 2
      return rounds + 1;
    }
  }
};
