...
Hide Similar Problems (M) Binary Tree Level Order Traversal (E) Maximum Depth of
Binary Tree
*/
...
finally, return the min of lt and rt, since this need to find the minDepth of
the tree*/
...
class Solution {
 public:
  int minDepth(TreeNode* root) {
    ...
    if (lt == 0 || rt == 0)
      return 1 + lt + rt;  // need to sepearate thinking about the left node and
                           // the right node
    else
      return 1 + min(lt, rt);
  }
