...
Hint 1:
Think in reverse; instead of finding the minimum prefix + suffix, find the
maximum subarray.
...
- once we find cur > target, move left pointer to check if existing cur ==
target case, if exist, compare (right-left+1) wih current res
