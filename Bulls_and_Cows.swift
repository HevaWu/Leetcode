...
class Solution {
    func getHint(_ secret: String, _ guess: String) -> String {
        ...
        var sArr = Array(repeating: 0, count: 10)
        var gArr = Array(repeating: 0, count: 10)
        ...
        for i in secret.indices {
            if secret[i] == guess[i] {
                ...
                sArr[secret[i].wholeNumberValue!] += 1
                gArr[guess[i].wholeNumberValue!] += 1
            }
        }

        for d in 0...9 {
            y += max(0, min(sArr[d], gArr[d]))
        }
