...
class Solution {
    func isPossible(_ nums: [Int]) -> Bool {
        ...
        // key: finded subsequence with last element as key-1
        // value: number of subsequence with last element as key-1
        var tail = [Int: Int]()

        // key is number, value is frequency of number
        var freq = [Int: Int]()
        ...
        for num in nums {
            if freq[num, default: 0] == 0 {
                continue
            } else if tail[num, default: 0] > 0 {
                // can append num to exist subsequence
                freq[num, default: 0] -= 1
                tail[num, default: 0] -= 1

                // update new tail
                tail[num+1, default: 0] += 1
            } else if freq[num+1, default: 0] > 0 && freq[num+2, default: 0] > 0 {
                // put num, num+1, num+2 as new subsequence
                freq[num, default: 0] -= 1
                freq[num+1, default: 0] -= 1
                freq[num+2, default: 0] -= 1

                // update new tail
                tail[num+3, default: 0] += 1
            } else {
                // cannot put num in exist subsequence
                // also cannot generate new subsequence
                return false
            }
        }
