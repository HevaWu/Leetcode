...
Space complexity : O(n). One candies array of size nn is used.
*/
class Solution {
...
class Solution {
    func candy(_ ratings: [Int]) -> Int {
        ...
        for i in stride(from: ratings.count-2, through: 0, by: -1) {
            // need check if candy is not assign properly
...
/*
Solution 4:
Coming to the implementation, we maintain two variables old_slope and new_slope to determine the occurence of a peak or a valley. We also use upup and downdown variables to keep a track of the count of elements on the rising slope and on the falling slope respectively(without including the peak element). We always update the total count of candies at the end of a falling slope following a rising slope (or a mountain). The leveling of the points in rankings also works as the end of a mountain. At the end of the mountain, we determine whether to include the peak point in the rising slope or in the falling slope by comparing the upup and downdown variables up to that point. Thus, the count assigned to the peak element becomes: max(up,down)+1. At this point, we can reset the upup and downdown variables indicating the start of a new mountain.
//
