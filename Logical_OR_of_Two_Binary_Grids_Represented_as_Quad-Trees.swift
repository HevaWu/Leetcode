...
class Solution {
    func intersect(_ quadTree1: Node?, _ quadTree2: Node?) -> Node? {
        if quadTree1 == nil, quadTree2 == nil { return nil }
        if let node1 = quadTree1, let node2 = quadTree2 {
            if node1.isLeaf { return node1.val ? node1 : node2 }
            if node2.isLeaf { return node2.val ? node2 : node1 }

            var node = Node(true, false)
            node.topLeft = intersect(node1.topLeft, node2.topLeft)
            node.topRight = intersect(node1.topRight, node2.topRight)
            node.bottomLeft = intersect(node1.bottomLeft, node2.bottomLeft)
            node.bottomRight = intersect(node1.bottomRight, node2.bottomRight)
