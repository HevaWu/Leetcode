...
used. The number of difference will be less than 3^N. The only case to reach 3^N
  ...
  int tallestBillboard(vector<int>& rods) {
    ...
    for (int rod : rods) {
      ...
      for (const auto& it : tmp) {
        int k = it.first;
        int v = it.second;
        dp[k + rod] = max(v, dp.find(rod + k) != dp.end() ? dp[rod + k] : 0);
        dp[abs(rod - k)] =
            max(v + min(rod, k),
                dp.find(abs(rod - k)) != dp.end() ? dp[abs(rod - k)] : 0);
      }
    }
    return dp[0];
  }
