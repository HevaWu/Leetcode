...
Time Complexity: O(W^{\frac{1}{4}} * \log W), where W = 10^{18} is our upper limit for RR. The \log WlogW term comes from checking whether each candidate is the root of a palindrome.
...
class Solution {
    func superpalindromesInRange(_ left: String, _ right: String) -> Int {
        ...
        for i in 1..<limit {
            ...
            if oddStrNum >= left, isPalindrome(oddStrNum) {
                finded += 1
            }
        }
        ...
        for i in 1..<limit {
            ...
            if evenStrNum >= left, isPalindrome(evenStrNum) {
                finded += 1
            }
        }
