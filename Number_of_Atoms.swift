...
/*
Solution 2:
Similar logic as Solution 1, but store as data structure
    ...
    func countOfAtoms(_ formula: String) -> String {
        ...
        while index < n {
            ...
            if c == "(" {
                ...
                if num == 0 { num = 1 }
                curGroup.applyCountToAllAtoms(num)
                curGroup.combineWithAnotherGroup(groupStack.removeLast())
            } else {
                var atom = Atom()
                ...
                while index < n, formula[index].isLowercase {
                    atom.name.append(formula[index])
                    index += 1
                }
                ...
                curGroup.addAtom(atom)
            }
        }

        // in final, only curGroup should contain value
        var seq = String()
        let finalFreq = curGroup.atomFreq
        let sortedAtomName = finalFreq.keys.sorted()
        for name in sortedAtomName {
            seq.append(name)
            if let atomCount = finalFreq[name], atomCount > 1 {
                seq.append(String(atomCount))
            }
        }
        return seq
    }
...
class Group {
    var atomFreq = [String: Int]()

    func addAtom(_ atom: Atom) {
        self.atomFreq[atom.name, default: 0] += atom.count
    }

    func applyCountToAllAtoms(_ count: Int) {
        for (atomName, atomCount) in atomFreq {
            atomFreq[atomName]! = atomCount * count
        }
    }

    func combineWithAnotherGroup(_ group: Group) {
        for (atomName, atomCount) in group.atomFreq {
            atomFreq[atomName, default: 0] += atomCount
        }
    }
}
