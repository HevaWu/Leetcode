...
class Solution {
    func decodeString(_ s: String) -> String {
        ...
        for c in s {
            if c.isNumber {
                number = number * 10 + c.wholeNumberValue!
            } else if c == Character("[") {
                pendRepeat.append(number > 0 ? number : 1)
                number = 0

                pendStr.append(cur)
                cur = ""
            } else if c == Character("]") {
                let shouldRepeat = pendRepeat.removeLast()
                cur = pendStr.removeLast() + String(repeating: cur, count: shouldRepeat)
            } else {
                cur.append(c)
            }
        }
