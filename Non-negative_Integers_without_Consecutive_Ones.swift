...
- f[i] gives the count of such binary numbers with ii bits.
...
- start scanning the given number numnum from its MSB.
...
- For every 0 encountered, we don't add any factor. We also keep a track of the last bit checked.
  ...
  - If we don't find any two consecutive 1's, we proceed till reaching the LSB and add an extra 1 to account for the given number num as well, since the procedure discussed above considers numbers up to numnum without including itself.

Time complexity : O(log_2(max\_int)=32). One loop to fill ff array and one loop to check all bits of numnum.
