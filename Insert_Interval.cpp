...
through comparing s to each single interval.end and by comparing e to each single interval.start, 
we can know if this interval can be covered into new interval, if it covered, we reset current
s and e
if it is not, the left part we store into left vector, the right part we store into right vector
at the end, combine the left part, new interval and the right part as return  */
