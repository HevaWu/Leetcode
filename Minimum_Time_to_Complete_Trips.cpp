...
/*
Solution 1:
binary search get min time

Time Complexity: O(NlogN)
- N: minTime * totalTrips
...
class Solution {
 public:
  long long minimumTime(vector<int>& time, int totalTrips) {
    ...
    long long l = minTime;
    long long r = (long long)minTime * totalTrips;
    while (l < r) {
      long long mid = l + (r - l) / 2;
      if (getTrips(mid, time) < totalTrips) {
        l = mid + 1;
      } else {
        r = mid;
      }
  ...
  // return number of trips can be finished in totalTime
  long long getTrips(long long total, vector<int>& time) {
    long long trips = 0;
    for (int t : time) {
      trips += (total / t);
    }
