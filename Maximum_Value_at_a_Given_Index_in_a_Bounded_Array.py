...
class Solution:
    def maxValue(self, n: int, index: int, maxSum: int) -> int:
        ...
        while l+1 < r:
            mid = l + (r-l)//2
            if self.canFind(mid, n, index, maxSum):
                l = mid
            else:
                r = mid
        return l

    def canFind(self, val: int, n: int, index: int, maxSum: int) -> bool:
        minLeft = (index * (val - 1 + val - index)) // 2 if val - index >= 1 else (val * (val - 1)) // 2 + (index - val + 1)

        minRight = ((n-index-1) * (val - 1 + val - (n-index-1)))//2 if val - (n-index-1) >= 1 else (val * (val-1)) // 2 + (n-index-val)

        return minLeft + minRight <= maxSum - val
