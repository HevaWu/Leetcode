...
public:
    ...
    //     vector<int> course = makeCourse(graph);
    //     for(int i = 0; i < numCourses; i++){
    //         int j = 0;
    //         for(; j < numCourses; j++)
    //             if(!course[j]) break;
    ...
    //     vector<unordered_set<int> > graph(numCourses);
    //     for(int i = 0; i < prerequisites.size(); i++){
    //         graph[prerequisites[i].second].insert(prerequisites[i].first);  //build graph to store each prerequisites course of one course
    //     }
    //     /*
