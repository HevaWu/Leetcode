...
// In order to develop the relation, let's assume that f[n] represents the number of possible rewardable strings(with LL and PP as the only characters) of length nn. Then, we can easily determine the value of f[n] if we know the values of the counts for smaller values of nn. To see how it works, let's examine the figure below:
// The above figure depicts the division of the rewardable string of length nn into two strings of length n-1n−1 and ending with LL or PP. The string ending with PP of length nn is always rewardable provided the string of length n-1n−1 is rewardable. Thus, this string accounts for a factor of f[n-1] to f[n].
...
// Thus, accounting for the first string again, all the rewardable strings of length n-1n−1, except the strings of length n-4n−4 followed by PLLPLL, can contribute to a rewardable string of length nn. Thus, this string accounts for a factor of f[n-1] - f[n-4]f[n−1]−f[n−4] to f[n].
//
// f[n]=2f[n−1]−f[n−4]
...
// No AA is present: In this case, the number of rewardable strings is the same as f[n].
