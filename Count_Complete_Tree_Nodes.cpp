...
start from the left node, since it is a complete tree, search from both left and
right node if the right node is null, check if the left node is null, if not
there should be some node under the left node, count these node
if the left node is null, this node is a leaf node, we can traverse the
root.right*/
/**
