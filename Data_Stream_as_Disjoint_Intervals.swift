...
then iteratively check and merge 2 side

Time Complexity: O(logn + n)
- n, current intervals length
    ...
    func addNum(_ val: Int) {
        ...
        while index >= 0 {
            ...
            if cur[0] <= intervals[index][1] + 1 {
                // print("left", index, cur, intervals)
                cur[1] = max(
                    intervals[index][1],
                    cur[1]
                )
        ...
        // print("before right", val, cur, index, intervals)
        index += 1
        ...
        // print("done right", val, cur, index, intervals)
