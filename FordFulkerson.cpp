...
public:
	...
	int FindPath1(int s, int t){
		...
		if(subflow[t] == -1)
			return -1;
		// for(int i = 0; i < vsize; i++){
		// 	cout <<"i is: " << subflow[i] << " " << mflow[i] << endl;
		// }
		// cout << "The max flow is " << subflow[t];
		return mflow[t];
	}
		...
		while(!Q.empty()){
			...
			while(temp->vnode != NULL){
				if(temp->vnode->flow>0 && subflow[temp->vnode->nindex]==-1){
					...
					subflow[temp->vnode->nindex] = temp->vindex;
					// cout <<endl << "method2 " << temp->vnode->nindex << " " << temp->vindex << " " << mflow[temp->vnode->nindex] << endl;
