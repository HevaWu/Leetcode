...
class Solution {
    func maximumNumber(_ num: String, _ change: [Int]) -> String {
        ...
        for i in 0..<n {
            let curDigit = num[i].wholeNumberValue!
            let canChangeDigit = change[curDigit]
            // >=
            if canChangeDigit == curDigit {
                if preIndex == -1 {
                    // find 1st strictly increasing, then do next
                    continue
                } else if preIndex+1 == i {
                    preIndex = i
                }
            } else if canChangeDigit > curDigit {
                if preIndex == -1 || preIndex + 1 == i {
                    // update it to changedDigit
                    num[i] = String(canChangeDigit).first!
                    preIndex = i
                } else {
                    break
                }
