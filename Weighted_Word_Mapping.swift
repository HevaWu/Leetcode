...
/*
Solution 1:
iterate word, get its weight, then transfer to expected char

Time Complexity: O(mn)
n is the length of words
m is max length of each word
...
class Solution {
    func mapWordWeights(_ words: [String], _ weights: [Int]) -> String {
        var res = ""
        for word in words {
            let weight = getWeightOfWord(word, weights)
            res.append(getCharFromInt(weight))
        }
        return res
    }

    func getWeightOfWord(_ word: String, _ weights: [Int]) -> Int {
        var val = 0
    ...
    func getCharFromInt(_ val: Int) -> Character{
        var index = 26 - (val % 26)
        let a = Character("a").asciiValue!
        return Character(UnicodeScalar(Int(a)+index-1)!)
    }
}
